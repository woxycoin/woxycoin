...
bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header (skip for genesis block at pos 8)
    if (pos.nPos != 8 && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams))
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    bool isGenesis = block.hashPrevBlock.IsNull();
    if (fCheckPOW && !isGenesis && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams))
